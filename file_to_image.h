...
// open disk image file
	...
	if ((filestream=fopen(diskimage_name,"rb")) == NULL)
		...
		if ((filestream=fopen(diskimage_name,"wb")) == NULL) file_err("Can't create",diskimage_name);
					
		fwrite ( dos25_block     , header_len       , 1 , filestream);
		fwrite ( dos25_block + 16, DOS_BLOCK_SIZE-16, 1 , filestream );
		for ( i = DOS_BLOCK_SIZE-16+1;    i <= (360-1)*128;    i++ ) fputc('\0',filestream);
		fwrite ( dos25_votc_dir , DOS25_VOTC_DIR_SIZE, 1 , filestream );
		for ( i = (720-360-1)*128 + DOS25_VOTC_DIR_SIZE ;   i < 720*128 ;   i++ ) fputc('\0',filestream);
		
		fclose (filestream);
	
		if ((filestream=fopen(diskimage_name,"rb")) == NULL) file_err("Can't open",diskimage_name);
#endif	
		}
	...
	if (lenf > MAX_LEN + header_len) file_err("File is too long:",diskimage_name);
...
/***** Show ATR header **************************************/
	...
	if (header_len == 16)
	{
		...
		printf ("%2X",fgetc(filestream));puts (" * 01: $0296 (sum of NICKATARI) ");
		printf ("%2X",fgetc(filestream));puts (" * 02: ");
		printf ("%2X",fgetc(filestream));puts (" * 03: size of this disk image, in paragraphs (size/$10) , low");
		printf ("%2X",fgetc(filestream));puts (" * 04: size of this disk image, in paragraphs (size/$10) , high");
		printf ("%2X",fgetc(filestream));puts (" * 05: sector size. ($80 or $100) bytes/sector, low");
		printf ("%2X",fgetc(filestream));puts (" * 06: sector size. ($80 or $100) bytes/sector, high");
		printf ("%2X",fgetc(filestream));puts (" * 07: high part of size, in paragraphs (added by REV 3.00) ");
		printf ("%2X",fgetc(filestream));puts (" * 08..16: Unused");
		printf ("%2X",fgetc(filestream));puts (" * Remainder of image contains Atari data. ");
		...
		for (j=0; j<=112; j += 16)
			{
				...
				printf ("%-3i%11s Byte0:%3i%4i%4i\n",filno,as,(unsigned char)ds[i][j],i,j);
...
/**************** Read in final (partial) sector *****************************/
	...
	if (seccount-needsecs)//this is just a check
		{
		printf ("\aRequire %i sectors, but %i were written", needsecs,seccount);
		_getch();
		}
...
//	fseek (filestream, header_len , SEEK_SET);			// Reset file ptr
	...
	if (df == 2)
		secno = 1040;
	for (sec=1; sec<=secno; sec++)
		if (fwrite (&ds[sec][0],sectlen,1,filestream)!=1) puts("disk write error");
	fclose (filestream);
	printf ("...... done.\n\n");
#endif
...
/****************     Create ATR image from XFD image  ******************************************/
	...
	if (header_len == 0)
		...
		if (df == 2)
			secno = 1040;
		for (sec=1; sec<=secno; sec++)
			if (fwrite (&ds[sec][0],sectlen,1,filestream3)!=1) puts("disk write error");
		fclose (filestream3);
		}
