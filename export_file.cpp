...
/*********** Ganze Diskette laden *********************/
    ...
    if ((strcmp(file_extension, ".XFD") == 0)
        || (strcmp(file_extension, ".xfd") == 0))
        header_len = 0;
    else if ((strcmp(file_extension, ".ATR") == 0)
             || (strcmp(file_extension, ".atr") == 0))
        header_len = 16;
    else {
        ...
        for (j = 0; j <= 112; j += 16) {
            ...
            if (!(ds[i][j] & 128)) {
                ...
                do {
                    ...
                    sec =
                        ((unsigned char)ds[sec][sectlen - 2]) +
                        ((unsigned char)(ds[sec][sectlen - 3] & 3)) * 256;
                } while (sec);
#if SHOW_SECS
                puts("\n");
#endif
