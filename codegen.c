...
enum {OPC,NN,AA,AAAA,AAX,AAAAX,AAY,AAAAY,BAAXB,BAABY,BRA,BAAAAB,ERROR};
...
/*************************  global variables  ************/
char opcdat[] = "BITSTYLDYCPYCPXORAANDEORADCSTALDACMPSBCASLROLLSRRORSTXLDXDECINC";

unsigned short data[21][15] =	{
/* BIT */
         { 0,0,0x24,0x2C,0,0,0,0,0,
          0xD6,0x67,0x46,0x48,0x4A},

 /* STY */
         { 0,0,0x84,0x8C,0x94,0,0,0,0,0,
	       0xD6,0x67,0x46,0,0},

  /* LDY */

         { 0,0xA0,0xA4,0xAC,0xB4,0xBC,0,0,0,0,
          0xD6,0x67,0x46,0xD0,0x4A},

  /* CPY */

         { 0,0xC0,0xC4,0xCC,0,0,0,0,0,0,
          0xD6,0x67,0x46,0,0},

    /* CPX */

         { 0,0xE0,0xE4,0xEC,0,0,0,0,0,0,
          0xD6,0x67,0x46,0,0},

 /* ORA */

         { 0,9,5,0xD,0x15,0x1D,0,0x19,1,0x11,
          0xD6,0x67,0x46,0,0},

  /* AND */

         { 0,0x29,0x25,0x2D,0x35,0x3D,0,0x39,0x21,0x31,
          0xD6,0x67,0x46,0,0},

 /* EOR */

         { 0,0x49,0x45,0x4D,0x55,0x5D,0,0x59,0x41,0x51,
          0xD6,0x67,0x46,0,0},

 /* ADC */

         { 0,0x69,0x65,0x6D,0x75,0x7D,0,0x79,0x61,0x71,
          0xD6,0x67,0x46,0xBF,0x4A},

  /* STA */

         { 0,0,0x85,0x8D,0x95,0x9D,0,0x99,0x81,0x91,
          0xD6,0x67,0x46,0,0},

  /* LDA */

         { 0,0xA9,0xA5,0xAD,0xB5,0xBD,0,0xB9,0xA1,0xB1,
          0xD6,0x67,0x46,0,0},

 /* CMP */

         { 0,0xC9,0xC5,0xCD,0xD5,0xDD,0,0xD9,0xC1,0xD1,
          0xD6,0x67,0x46,0,0},

 /* SBC */

         { 0,0xE9,0xE5,0xED,0xF5,0xFD,0,0xF9,0xE1,0xF1, // first 10 bytes are possible codes
          0xD6,0x67,0x46,0x9E,0x49},

 /* ASL */

         { 0xA,0,6,0xE,0x16,0x1E,0,0,0,0,
          0xD6,0x67,0x46,0,0},

 /* ROL */

         { 0x2A,0,0x26,0x2E,0x36,0x3E,0,0,0,0,
          0xD6,0x67,0x46,0x37,0x4A},

   /* LSR */

         { 0x4A,0,0x46,0x4E,0x56,0x5E,0,0,0,0,
          0xD6,0x67,0x46,0xF3,0x49},

  /* ROR */

         { 0x6A,0,0x66,0x6E,0x76,0x7E,0,0,0,0,
         0xD6,0x67,0x46,0x8D,0x49},

  /* STX */

         { 0,0,0x86,0x8E,0,0,0x96,0,0,0, //In ATMAS Should be $96,0, not 0,$96,  -- STX aa,Y not STX aaaa,Y
         0xD6,0x67,0x46,0x15,0x4A},

 /* LDX */

         { 0,0xA2,0xA6,0xAE,0,0,0xB6,0xBE,0,0,
          0xD6,0x67,0x46,0,0},

 /* DEC */

         { 0,0,0xC6,0xCE,0xD6,0xDE,0,0,0,0,
          0xD6,0x67,0x46,0,0},

 /* INC */

         { 0,0,0xE6,0xEE,0xF6,0xFE,0,0,0,0,
          0,0,0,0,0}										};
...
char opcds[] =  " BIT STY LDY CPY CPX ORA AND EOR ADC STA LDA CMP SBC ASL ROL "
				"LSR ROR STX LDX DEC INC "
				"BRK PHP ASL CLC PLP ROL SEC RTI PHA LSR CLI RTS PLA ROR SEI "
				"DEY TXA TYA TXS TAY TAX CLV TSX INY DEX CLD INX NOP SED "
				"BPL BMI BVC BVS BCC BCS BNE BEQ JMP JSR ";// for checking labels

char assdirecs[] = " EQU EPZ ORG ASC DFB DFW DSKIMAGE XFDIMAGE ATRIMAGE RUNEMUL CPFL ORG APPEND RMB INITRMB "
...
unsigned short bradat[] = {0x10,0x30,0x50,0x70,0x90,0xb0,0xd0,0xf0};
int braoffset;
...
puts("*                                                                     *");
puts("***********************************************************************\n");
if (argc>1) strcpy(src_filename,argv[1]);
else {
	printf ("Please enter name of file to be assembled>");
	gets(src_filename);
	if (strchr(src_filename,'.')==0) strcat(src_filename,".src");
	}
//printf ("\n%s\n",src_filename);
...
for (pass=1; pass<=2; pass++)
	...
	for (linnum=0;	!((*txtptr == 0) && (include_flg == 0)) ;	 txtptr = linbegptr + linlen)
		{
		...
		if (strchr(outflgs,'L')||strchr(outflgs,'l'))
			...
			if (*prnlnbufbeg || *prnlnbuf )	
				...
				*prnlnbuf2[0] = '\0';
				...
				prnlnum = 0;
				data_flg = 0;
				for (i=0;i<10;i++) mismatchln_flg[i]=0;
				}	
			}
		
		if (!maclevel) linnum++;				//no change of line number if macro
		*label = 0;
		labflg = 0;
		*operand='\0';
			...
			*lnbfptr = 0;
			if ((*linptr == '@') && maclevel)
				...
				if (strchr(", \n+-",*ptr))	/*Characters which can precede a local label!!!!!!! */
					{
					if (cnt > 4)
						error ("Local labels cannot be longer than 5 characters");
					ptr++;
					labptr = lab;
					while(cnt)
						{
					...
					*labptr = 0;
					...
					for (loclabtbpos=0; loclabtbpos<loclabnum; loclabtbpos++)
						if (strcmp(loclabtb[loclabtbpos],lab) == 0) break; /*local label found*/
					...
					while (cnt < lclbnum[maclevel])
						{
						if (lclb[maclevel][cnt] == loclabtbpos) break;
						cnt++;
						}
...
/**********  Test first character of line ***********/
		...
		switch (*txtptr)
			...
			default	:					/* label			*/
				...
				if (*txtptr=='@')
					label[pos++] = *txtptr++;
				if (!isspace(*txtptr)) error ("Label must consist only of alphabetic and numeric characters");
				label[pos] = 0;
				
				sprintf(labelstr," %s ",label);
				if (strstr (opcds,        labelstr) ) error ("You can't have an opcode at the beginning of a line");
				if (strstr (assdirecs,    labelstr) ) error ("You can't have an assembler directive the beginning of a line");
				if (strstr (assdirecssolo,labelstr) ) error ("You can't have an assembler directive the beginning of a line");	
				}
			}
	...
	/*	********* find opcode ****************************/
		...
		tpsav = txtptr;
		if (*txtptr =='\n') continue;
		if (isalpha(*txtptr)==0) error("Alphabetic character expected");
		ptr = opcode;
		...
		*ptr = 0; //opcode finalised
		...
		if (     !(*txtptr == ' ') || *(txtptr+1)==' ' ||  *(txtptr+1)=='\n'|| *(txtptr+1)=='\t'      ) //Check operand for dirictives
			...
			if (strstr (assdirecs,opcstr)) 
				error ("Directive must be followed by a single space (not tab) and an operand");
			}
		...
		if ((pass == 2) && *outflgs)
			...
			if (strcmp(opcode,"MACRO") && strcmp(opcode,"MEND") && !macroflg) /* Not in macro */
				...
				if ((maclevel ==0) || !( strchr(outflgs,'M')||strchr(outflgs,'m') )) //M = macros not expanded
					...
					if (  !(strstr("EQU EPZ ORG",opcode)&&(strlen(opcode)==3))  )
						{
						if (pc != pc_sav)
...
/* LABEL AT LINE BEGIN  LABEL AT LINE BEGIN LABEL AT LINE BEGIN LABEL AT LINE BEGIN*/
		...
		if (*label)
			...
			if (pass == 1)
				{
				if (symnum >= MAXSYMS-1) error ("Too many labels");
				if (macnum >= MAXMACSYMS-1) error ("Too many labels");
				}

/* EPZ EQU  LABEL AT LINE BEGIN*/

			if ((strcmp(opcode,"EQU") == 0)||(strcmp(opcode,"EPZ") == 0))
				{
				txtptr++;
				i=expresstoi();
				if (pass == 1)
...
/* RMB  LABEL AT LINE BEGIN*/			//(Init ving memory bytes)
			...
			if (strcmp(opcode,"RMB") == 0)
				{
				if (!rmbflg)error ("INITRMB value must precede this line");
...
/* MACRO  LABEL AT LINE BEGIN*/
			...
			if (strcmp(opcode,"MACRO") == 0)							// MovAl   MACRO A,N
				{
				if (macroflg) error ("MEND missing");
				macroflg++;
				if (pass == 1)
					{
					find_operand();										// A,N
...
/*  LABEL AT LINE BEGIN IN CODE */
				...
				{
				labflg = 1;
				if (strcmp(label,"DO"  ) == 0) sprintf (label, "DO%4.4i",  ++donum);
				if (strcmp(label,"LOOP") == 0) sprintf (label, "LOOP%4.4i",++loopnum);
				if (strcmp(label,"MINUS")== 0) sprintf (label, "MINU%4.4i",++minusnum);
				if (strcmp(label,"PLUS") == 0) sprintf (label, "PLUS%4.4i",plusnum++);
				if (strcmp(label,"SKIP") == 0) sprintf (label, "SKIP%4.4i",skipnum++);
				if (strcmp(label,"CONT") == 0) sprintf (label, "CONT%4.4i",contnum++);
				if (strcmp(label,"ENDIF")== 0) sprintf (label, "ENDI%4.4i",endifnum++);
				}
			if ((pass == 1 )&&!(macroflg))
				{
				strcpy (sss[symnum].symbol, label);
...
/* MEND */
		...
		if (strcmp(opcode,"MEND") == 0)
			...
			if (macroflg)
				...
				{
				if (!maclevel) error("MEND without MACRO");
				lclbnum[maclevel] = 0;
...
/* LINK */
		...
		if (strcmp(opcode,"LINK") == 0)
		{
			if (pass == 2)
			{
				if (!linkfile_filename) error ("LINKFILE must be designated before this line");
...
/* INCLUDE */
		...
		if (strcmp(opcode,"INCLUDE") == 0)
			{
			if (include_flg) error ("Can't have 'INCLUDE' in an included file.");
			include_flg = 1;
			operand_to_filename(include_filename);
			
			printf ("\nIncluding %s",include_filename);
		
			if ((stream3=fopen(include_filename,"r")) == NULL)error("Can't open INCLUDE file");
			includebuf = txtendptr;		
			include_txtsiz = fread(includebuf, 1 ,MAX_TXT_SIZ - txtsiz,stream3);
			fclose(stream3);
		
			txtsiz += include_txtsiz;
			if (txtsiz > MAX_TXT_SIZ-txtsiz) error ("INCLUDE File is too long");
...
/* ADDTOIMG  SMTFILE  RUN_EMUL  DSKIMAGE */
		...
		if	(pass == 2)	
			...
			if (strcmp(opcode,"ADDTOIMG") == 0)
				{operand_to_filename(addtoimg_filename[addtoimg_cnt++]);continue;}
			if (strcmp(opcode,"SMTFILE") == 0)
				{operand_to_filename(smbfilename);continue;}
			if (strcmp(opcode,"RUNEMUL") == 0)
				{operand_to_filename(emul_filename);continue;}
			if ((strcmp(opcode,"DSKIMAGE") == 0)||(strcmp(opcode,"XFDIMAGE") == 0)||(strcmp(opcode,"ATRIMAGE") == 0))
				{operand_to_filename(dsk_filename);continue;}	
			}

/* SAVEAS */

		if ((strcmp(opcode,"SAVEAS") == 0)||(strcmp(opcode,"BSAVE") == 0))
			{
			if (pass == 1)
...
/* NOHEADER */
		...
		if ((strcmp(opcode,"NOHEADER") == 0)||(strcmp(opcode,"OBJFILE") == 0))
			{
			objfile_flag = 1;
			continue;
			}
...
/* ORG */
		...
		if (strcmp(opcode,"ORG") == 0)
			...
			txtptr++;
				...
				if (!orgflg)											/* first ORG */
					{
					if (*txtptr=='*') error ("ORG value is undefined");
					orgflg = 1;
					}
				else
					{
...
/* DFB */
		...
		if (strcmp(opcode,"DFB") == 0)
			{
			data_flg = 1;
			if (*(txtptr+1) == '%')	sectyp('%');else sectyp('#');
			do	{
				txtptr++;
...
/* FILLMEM */
		...
		if (strcmp(opcode,"FILLMEM") == 0)
			...
			txtptr++;
			cnt = expresstoi_p2();
			if (*txtptr != ',') error ("Comma followed by byte value expected");
			txtptr++;
...
/* OPCODE IS A MACRO */
		...
		for (i=0;i<macnum;i++)
			if (strcmp(macsymbol[i],opcode)== 0) break;
		if (i < macnum)
			{
...
/* OPC without operand */
		...
		if (!(*operand))
			...
			if ((cnt=chkopc(opcdat1)) >=0) /* rts, sec, plp, ...... */
				...
				{
				if (strlen(opcode) == 3)
					if (strstr (opcds,opcode)) error ("Opcode must be followed by a single space (not tab) and an operand");
...
/* JMP */
		...
		if (strcmp(opcode,"JMP")==0)
			...
			if (*operand == '(')
				{
				if (operand[strlen(operand)-1] != ')') error ("Illegal syntax or addressing");
				toxlbuf(108);
				}
			else
				toxlbuf(76);
		...
		if ((cnt=chkopc(brastrdat)) >=0)
			...
			if (pass == 2)
				...
				if (temp <-128 || temp >127)
					{
					printf("\nAttempted to branch a distance of %i. (Range -128 to 127 is permitted.)",temp);
...
/* JSR */
		...
		if (!strcmp(opcode,"JSR"))
			{
			if (strchr(";'!(",section_type) == 0) sectyp(';');						//version 1.01
			symval =expresstoi_p2();
			toxlbuf( 0x20 );
			toxlbuf((char) symval );
			toxlbuf((char) (symval >> 8));
...
/* opc 'c */
		...
		if (operand[0] == '\'')
			{
			sectyp('\'');
			if (data[cnt][NN]== 0) error("direct addressing (') cannot be used with this opcode");
			txtptr++;
			ch = *txtptr++;
			if (*txtptr == '\'')
				txtptr++;
...
/* opc #nn */
		...
		if (operand[0] == '#')
			...
			if (strchr("!&@(",section_type) == 0) sectyp(';');
			toxlbuf( (char)  data[cnt][NN] );
			toxlbuf((char) intval );
			continue;
			}
...
/***********  Commands in data block  ***************/
		...
		if ((symval < 256) && (double_byte_flg==0))	// txtptr -> first byte after expression
			...
			if ( (*(txtptr-1) == ')') && (*operand == '(')) txtptr--;	//needed for expresstoi(new)
			
			switch (*txtptr)
				{
				case ' ':							
				case '\t':
				case '\n':   operno = AA;break;     //OPC without operand
				case ',':
					switch (*(++txtptr))
						{
						case 'Y':					//   ,Y
						...
						case 'x':
							...
							switch (*(++txtptr))
								{
								case ')':			//  OPC (aa,X)
									if (*operand != '(') error ("Forward bracket missing");
									operno = BAAXB;break;
								default: 
									operno = AAX;break; // OPC aa,X
								}
							break;
						default: error("Must have X or Y after comma -- ");break;
						}
					break;
				case ')':
					if (*operand != '(') error ("Forward bracket missing");
					switch (*(++txtptr))
						...
						case ',':
							switch (*(++txtptr))
								...
								case 'Y':
								case 'y':
									if ((*(opcode+2)=='Y') || (*(opcode+2)=='X'))
										error ("Can't use indirect addressing with this opcode");
									
									operno = BAABY;break; // OPA (AA),Y
								default:error("Must have Y after comma for indirect addressing");break;
								}
							break;
						default: error("Comma missing after bracket");break;
						}
					break;
				default: printf ("Unexpected character: '%c' -- ",*txtptr);error("Symbol contains illegal character");break;
				}
			...
			{
			if (*operand == '(') 
				error ("Zero page address expected -- Operand not defined -- Label may be unknown");
				...
				case ',':
					switch (*(++txtptr))
						...
						case 'x':
							...
							operno = AAAAX;break;
						default: error("Must have X or Y after ','");break;
						}
					break;
		...
		if (pass == 2)
			if (*xlbfptr != (char) data[cnt][operno])
				{
				printf("\n\nOpcode mismatch %X <> %X",0xff&(*xlbfptr), 0xff&data[cnt][operno]);
				error ("This zero-page label must be defined before present line.");
				}
		if (data[cnt][operno]==0) error("This addressing is not possible with this opcode");
	...
	if (pass == 1)
		...
		for (i=0; i<macnum; i++)
			{
			if (symnum >= MAXSYMS-1) err ("Too many labels");
			if (macptrtab[i])
...
if (lstflg) fclose(stream2);

puts("");
symnum = 0;
for (i=0; i<symnumext; i++)
	{
	if (sss[i].macf == MACRO) continue;
	if ( ((sss[i].macf == UNUSED)||(sss[i].macf == MACRO_UNUSED)) && sss[i].symbol[0] )
	...
	if ((sss[i].symadr == sss[symnum-1].symadr)&&(symnum>0))	/* double entry */
		...
		if (sss[symnum-1].symbol[0] && sss[i].symbol[0]) /* 2 labels */
			{
			if ((sss[symnum-1].macf == MACRO_LAB)||(sss[i].macf == MACRO_LAB))
				{
				...
				printf("%s and %s both = $%X\n",sss[i].symbol,sss[symnum-1].symbol,sss[i].symadr);
			}
...
if(*append_filename)								
		...
		for (i=0;i<append_cnt;i++)
			...
			if ((txtsiz = fread(xlbfptr,1,MAX_BIN_SIZ-(xlbfptr-xlbuf),stream)) == 0)
				{
				printf("file size: %i\n",txtsiz);
...
if(*linkfile_filename)								
	...
	for (i=0;i<link_cnt;i++)
		...
		txtsiz = fread(xlbfptr,1,MAX_BIN_SIZ-(xlbfptr-xlbuf),stream);
		if (txtsiz == 0)
...
if (cpfflg)
	...
	if (mismatch_flg)
		{
		printf("\a******* CPFL - MISMATCH at address $%X, line %i *******\n",mismatch_pc-1,mismatch_linnum);
		}
	else
		{
		puts ("");
...
if(*operand == '(') *ptr++ = '('; //because txtptr++ if beginning bracket
...
do
	...
	switch (*txtptr)
		...
		default:
			...
			if ( (symval_list[symval_cnt] = symtoi1() )==NOTFOUND) notfound = 1;
			*ptr++ = symval_cnt++ + 'A';
		}
					...
					if(infix[i]!=')' && infix[i]!='(')
						...
						if(infix[i]==')')
							...
							while(temp!='(')
								{
								postfix[j++]=temp;
								if (tos==0) error("Missing opening parenthisis");
								temp=stack[--tos];
								}
							}
						}
...
ptr = postfix;
		while(*ptr)
			...
			if (isalpha(*ptr))
				...
				switch (*ptr)
					{
					case '+': intstack[tos++] = a + b;break;
					case '-': intstack[tos++] = a - b;break;
					case '*': intstack[tos++] = a * b;break;
					case '/': intstack[tos++] = a / b;break;	   
					default :
						if (*ptr == '(') error ("Missing closing parenthisis");
						printf("infix: %s   -Character '%c' in %s  operand is %s",infix, *ptr,postfix,operand);
		...
		if (strstr (opcode,"),Y")) txtptr--;
		result = intstack[--tos];

//		if (tos) error ("Illegal syntax in expression");
	if (tos) {
			
			printf ("\n\noperand: %s   infix: %s   postfix: %s",operand,infix,postfix);
...
if (ch == '%')					/*  %:Binary */
	{
	txtptr++;
	if (!(*txtptr =='0' || *txtptr =='1')) error ("Binary digit 0 or 1 expected.");
	ch = 0;
	chrcnt = 1;
	while (*txtptr =='0' || *txtptr =='1')
		{
		if (chrcnt++ > 8) error ("Too many binary digits. Maximum number of digits is 8. e.g. %10010011");
		ch <<= 1;
		ch += *txtptr++ - '0';
		}
	return(ch);
...
if (ch == '$')					/*  $:Hex */
	{
	txtptr++;
	if (isxdigit(*txtptr)==0) error ("Hexidecimal digit expected");
	chrcnt = 1;
	while (isxdigit(*txtptr))
		{
		if (chrcnt++ > 4) error ("Too many hexdecimal digits. Maximum number of digits is 4 eg. $1234.");
		*ptr++ = *txtptr++;
		}
	*ptr = '\0';
	sscanf(strbeg,"%x",&i);
...
switch (ch)
	{
	case '+' : strcpy (strbeg,"PLUS"); break;
	case '-' : strcpy (strbeg,"MINUS"); break;
	default  : //label
		if (isspace(*txtptr)) error ("Ilegal space, tab, or end of line");
		if (!(isalpha(*txtptr))) error ("Label must begin with a letter");
...
if (pass == 1)
	...
	{
	if ((strcmp(strbeg,"DO")==0))   sprintf (strbeg, "DO%4.4i\0"  ,donum  );
	if ((strcmp(strbeg,"LOOP")==0)) sprintf (strbeg, "LOOP%4.4i\0",loopnum);
	if ((strcmp(strbeg,"MINUS")==0))sprintf (strbeg, "MINU%4.4i\0",minusnum);
	if ((strcmp(strbeg,"PLUS")==0))	sprintf (strbeg, "PLUS%4.4i\0",plusnum);
	if ((strcmp(strbeg,"SKIP")==0))	sprintf (strbeg, "SKIP%4.4i\0",skipnum);
	if ((strcmp(strbeg,"CONT")==0))	sprintf (strbeg, "CONT%4.4i\0",contnum);
	if ((strcmp(strbeg,"ENDIF")==0))sprintf (strbeg, "ENDI%4.4i\0",endifnum);

	if ((sssptr=bsearch(strbeg, sss, symnum, sizeof(ssstype), strcmp))!=0)
		{
		if (maclevel)
...
void toxlbuf(char ch)
	{
	char tempstr[10];
	if (!orgflg)error ("ORG value must precede this line");
	...
	*xlbfptr = ch;
	...
	if (pass == 2)
		...
		if ((strchr(outflgs,'L')||strchr(outflgs,'l')) && !( strchr(outflgs,'M')||strchr(outflgs,'m') ))
			{
...
void operand_to_filename(char filename[])
	...
	switch(*txtptr++)
		{
		case '\n'	:
		case '\t'	: error("Filename missing");	break;
		case ' '		:
			linptr = txtptr;
			if (!strchr(" \t\n",*txtptr))	/* there is an operand */
			{
...
if ( (line_type != section_type) || labflg ) // if new sec type or a label
	...
	if (labflg)								//if there is a label....
		...
		if (pass == 1) /**/
			...
			else sss[symnum-1].symtyp = line_type; // else make entry
			}
		if ((pass == 2) && (line_type == '&')) /**/
			{
		...
		if ((pass == 2)&&(pc > 0xff))
			{
			if (symnumext >= MAXSYMS) error ("Too many symbols");
